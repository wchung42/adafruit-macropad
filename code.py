...
while True:
    ...
    if lighting_modes[current_lighting_mode] == 'RAINBOW':
        rainbow.animate()
    elif lighting_modes[current_lighting_mode] == 'CHASE':
        rainbow_chase.animate()
    elif lighting_modes[current_lighting_mode] == 'COMET':
        rainbow_comet.animate()
    elif lighting_modes[current_lighting_mode] == 'SPARKLE':
    ...
    if pressed:
        ...
        if lighting_modes[current_lighting_mode] != 'MACROS' and key_number < 12: # No pixel for encoder button
            macropad.pixels[key_number] = 0xFFFFFF
            macropad.pixels.show()
        for item in sequence:
            if callable(item):
                returnValue = item(macropad)
                if returnValue is not None:
                    current_lighting_mode = returnValue
    ...
    # Macro-specific lighting
    if lighting_modes[current_lighting_mode] == 'MACROS':
        for i in range(12):
            if pressed and i == key_number:
                macropad.pixels[i] = 0xFFFFFF
            else:
                macropad.pixels[i] = apps[app_index].macros[i][0]
            macropad.pixels.show()
    
    if lighting_modes[current_lighting_mode] == 'LIGHTSOUT':
        for i in range(12):
            if pressed and i == key_number:
                macropad.pixels[i] = 0xFFFFFF
            else:
